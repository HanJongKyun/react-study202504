...
const loginReducer = (state, action) => {
  switch (action.type) {
    ...
    case 'EMAIL_VALID':
      return {
        ...state,
        emailIsVaild: state.enteredEmail.includes('@'),
      };
    case 'PASSWORD_VALID':
      return {
        ...state,
...
const Login = ({ onLogin }) => {
  ...
  const { enteredEmail, enteredPw, emailIsVaild, pwIsValid } = loginState;
  ...
  return (
    <Card className={styles.login}>
      <form onSubmit={submitHandler}>
        <div
          className={`${styles.control} ${!emailIsVaild ? styles.invalid : ''}`}
        >
        ...
        <div className={styles.actions}>
          ...
          <Button
            ...
            disabled={!(emailIsVaild && pwIsValid)}
          >
            Login
          </Button>
        </div>
